...
//------------------------------------------------------------------------------
void FV::reconstruct_first (const std::vector<double> & state,
							double & rec_statel,
							double & rec_stater) const
{
   int s = state.size();
   assert(s>=2);
   rec_statel = state[s/2-1];
   rec_stater = state[s/2];
}
...
//------------------------------------------------------------------------------
void FV::reconstruct_second_muscl (const std::vector<double> & state,
							       double & rec_statel,
							       double & rec_stater) const
{
   ...
   dstate    = state[s/2]  - state[s/2-1]  ;
   dstatel   = state[s/2-1]  - state[s/2-2]  ;
   dstater   = state[s/2+1]  - state[s/2]  ;
   
   // left state
   rec_statel  = state[s/2-1]  + (dstatel*(1.0-KKK) + dstate*(1.0+KKK))*0.25;
   
   // right state
   rec_stater  = state[s/2]  - (dstater*(1.0-KKK) + dstate*(1.0+KKK))*0.25;
}
							...
							double & rec_stater) const
{
   ...
   {
	  state_comp[j] = state[s/2 - eno_level + j] ;
   }
   rec_statel =eno_pw(eno_level,state_comp);
   
   // right state
   for(int j=0; j<2*eno_level-1; ++j)
   {
	  state_comp[2*eno_level-2 -j] = state[s/2 - eno_level + 1 + j] ;
   }
   rec_stater =eno_pw(eno_level,state_comp);
							...
							double & rec_stater) const
{
   ...
   for(int j=0; j<4; ++j)
   {
	  state_comp[j] = state[s/2 - 2 + j] ;
   }
   sp_weno_rec(state_comp,rec_statel ,rec_stater ,param.spweno_corr_type);
...
// Reconstruct left and right states using point wise SP-WENO
//------------------------------------------------------------------------------
void FV::reconstruct_sp_weno_dl(const std::vector<double> & state,
   double & rec_statel,
   double & rec_stater) const
{
   int s = state.size();
   ...
   if (abs(state_comp[2] - state_comp[1]) < 1e-15)
      rec_statel = 0.0, rec_stater = 0.0;
   else
      net.forward(state_comp,rec_statel,rec_stater);
}
